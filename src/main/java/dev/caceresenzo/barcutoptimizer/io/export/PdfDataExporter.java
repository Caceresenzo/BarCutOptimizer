...
import dev.caceresenzo.barcutoptimizer.optimize.impl.FillingCutAlgorithm;
import dev.caceresenzo.barcutoptimizer.ui.component.BarCutPanel;
import lombok.Data;
...
public class PdfDataExporter implements DataExporter {
	...
	private void printFooter(PDPageContentStream contentStream, PDRectangle mediaBox) throws IOException {
		...
		String[] lines = I18n.string("application.copyright.full", Defaults.VERSION).split("\n");
		for (int index = 0; index < lines.length; index++) {
	...
	static List<CutTableInput> getRandomCuts() {
		List<CutTableInput> inputs = new ArrayList<>();
